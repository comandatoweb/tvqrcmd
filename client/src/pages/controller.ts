...
export function initControllerPage(root: HTMLElement, roomId: string, token?: string): void {
  ...
  document.getElementById('pause')?.addEventListener('click', () => sendPause());
  document.getElementById('resume')?.addEventListener('click', () => sendPlayAt({ epochMs: Date.now() }));
  document.getElementById('forceStart')?.addEventListener('click', () => sendPlayAt({ epochMs: Date.now() + 100 }));
...
function updateState(state: RoomState, srcInput: HTMLInputElement, typeSelect: HTMLSelectElement, seekInput: HTMLInputElement) {
